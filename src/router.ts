...
export interface NxpressDataModule {
  props?: (req: Request, res: Response) => Promise<Record<string, any>> | Record<string, any>;
  default?: (req: Request, res: Response) => Promise<Record<string, any>> | Record<string, any>;
}
 ...
 */
export function renderTemplateFile(filePath: string, props: Record<string, any>): string {
...
export function findLayoutsForRoute(
  ...
  const dirParts = path.dirname(templateRelPath).split(/[/\\]/).filter((p) => p !== '.' && p !== '');
...
export function fileToRoutePath(relPath: string): string {
  let routePath = relPath.replace(/\.(hbs|ejs|html|pug|mustache|njk|js|ts)$/, '');
 ...
 */
export function registerRoutes(app: Express, appDir: string, options: RouterOptions = {}): void {
  ...
  const files = globSync('**/*.{hbs,html,ejs,pug,mustache,njk,js,ts}', { cwd: appDir });
  ...
  const templateFiles = pageFiles.filter((f) => !f.endsWith('.js') && !f.endsWith('.ts'));
